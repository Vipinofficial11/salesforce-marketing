...
Feature: Salesforce Marketing Cloud Source - Run time Scenarios (macros)
  ...
  Scenario : Verify user should be able to preview the pipeline when plugin is configured for Single Object Data Retrieval mode with macros
    ...
    #And Enter runtime argument value "  " for key "queryMode"
    #This step can only take values from plugin properties page but what if I want to pass a string normally
    And Enter runtime argument value " " for key "objectName"
    And Enter runtime argument value "singleobject.externalkey" for key "dataExtensionKey"
    And Enter runtime argument value "   " for key "filter"
  ...
  #Create one test for multi use Example :  for multi object mode
  ...
  Scenario : Verify user should be able to preview the pipeline when plugin is configured for Multi Object Data Retrieval mode with macros
    ...
    And Preview and run the pipeline
    And Enter runtime argument value "  " for key "queryMode"
    And Enter runtime argument value "  " for key "objectList"
    And Enter runtime argument value "  " for key "dataExtensionKey"
    And Enter runtime argument value "  " for key "tableNameField"
    And Enter runtime argument value "   " for key "filter"
    And Enter runtime argument value "admin.clientid" for key "clientId"
    And Enter runtime argument value "admin.clientsecret" for key "clientSecret"
    And Enter runtime argument value "admin.authenticationbase.uri" for key "authEndpoint"
